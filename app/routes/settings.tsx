...
export function meta({}: Route.MetaArgs) {
  return [
    ...
    { name: "description", content: "Configure appearance, table display preferences, SQL options, and local browser storage." },
  ];
}
  ...
  return (
    <div className="flex items-center justify-between gap-4 py-3">
      <div className="space-y-0.5">
        <label className="text-xs sm:text-sm font-semibold text-slate-800 dark:text-slate-200 block cursor-pointer select-none" onClick={() => !disabled && onChange(!checked)}>
          {label}
        </label>
        {description && (
...
export default function SettingsPage() {
  ...
  const previewRows = [
    { id: 101, name: 'Alice Walker', role: 'Staff Engineer', dept: 'Core Platform', score: 98 },
    { id: 102, name: 'Marcus Chen', role: 'Product Designer', dept: 'Design Systems', score: null },
    { id: 103, name: 'Sarah Connor', role: 'Security Lead', dept: 'SecOps', score: 94 },
  ];
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-slate-400 font-medium">
                  Current: <strong className="capitalize text-emerald-600 dark:text-emerald-400">{settings.rowDensity}</strong>
                </span>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400">
              ...
              <div className="grid grid-cols-3 gap-3 pt-1">
                {[
                  { id: 'compact' as RowDensity, label: 'Compact', badge: '32px', note: 'Max records' },
                  { id: 'standard' as RowDensity, label: 'Standard', badge: '40px', note: 'Balanced' },
                  { id: 'relaxed' as RowDensity, label: 'Relaxed', badge: '48px', note: 'Spacious' },
                ].map((item) => {
                    ...
                    >
                      <div className="flex items-center justify-center gap-1.5">
                        ...
                        <span className={`text-[10px] px-1.5 py-0.2 rounded font-mono ${
                          isSelected ? 'bg-emerald-200/60 dark:bg-emerald-800/60 text-emerald-900 dark:text-emerald-100' : 'bg-slate-200/60 dark:bg-slate-700/60 text-slate-600 dark:text-slate-300'
                        }`}>
                          {item.badge}
                        </span>
                      </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-100/70 dark:bg-slate-900/80 border-b border-slate-200 dark:border-slate-800 text-[11px] font-bold text-slate-600 dark:text-slate-300">
                      ...
                      <th className="px-3 py-2 border-r border-slate-200 dark:border-slate-800">id</th>
                      <th className="px-3.5 py-2 border-r border-slate-200 dark:border-slate-800">name</th>
                      <th className="px-3.5 py-2 border-r border-slate-200 dark:border-slate-800">role</th>
                      <th className="px-3.5 py-2 border-r border-slate-200 dark:border-slate-800">department</th>
                      <th className="px-3.5 py-2">score</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800/70">
                    {previewRows.map((row, idx) => (
                      <tr key={row.id} className="hover:bg-emerald-50/30 dark:hover:bg-emerald-950/20 transition-colors">
                        {settings.showRowNumbers && (
                        ...
                        <td className={`${densityPaddingMap[settings.rowDensity]} font-mono text-emerald-600 dark:text-emerald-400 border-r border-slate-100 dark:border-slate-800/60`}>
                          {row.id}
                        </td>
                        <td className={`${densityPaddingMap[settings.rowDensity]} font-semibold text-slate-800 dark:text-slate-200 border-r border-slate-100 dark:border-slate-800/60`}>
                          {row.name}
                        </td>
                        <td className={`${densityPaddingMap[settings.rowDensity]} text-slate-600 dark:text-slate-400 border-r border-slate-100 dark:border-slate-800/60`}>
                          {row.role}
                        </td>
                        <td className={`${densityPaddingMap[settings.rowDensity]} text-slate-600 dark:text-slate-400 border-r border-slate-100 dark:border-slate-800/60`}>
                          {row.dept}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <span className="text-[11px] text-slate-400">
                    {storageUsage.indexedDbKb} KB in IndexedDB + {storageUsage.localStorageKb} KB in localStorage
                  </span>
                </div>
              </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-rose-700/80 dark:text-rose-300/80 mt-0.5">
                        Permanently deletes all imported datasets, SQL query logs, saved queries, and restores default settings.
                      </p>
                    </div>
                  </div>
